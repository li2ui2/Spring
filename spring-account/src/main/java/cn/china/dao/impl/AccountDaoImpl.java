...
import cn.china.utils.ConnectionUtils;
...
public class AccountDaoImpl implements IAccountDao {
    ...
    public void saveAccount(Account account) {
        try{
            runner.update(connectionUtils.getThreadConnection(),
                    "insert into account2(name,money)values(?,?)",
    ...
    public void updateAccount(Account account) {
        try{
            runner.update(connectionUtils.getThreadConnection(),
                    "update account2 set name=?,money=? where id=?",
                    account.getName(),
                    account.getMoney(),
                    account.getId());
        }catch (Exception e) {
    ...
    public Account findAccountByName(String accountName) {
        try{
            List<Account> accounts = runner.query(connectionUtils.getThreadConnection(),
                    "select * from account2 where name = ? ",
                    new BeanListHandler<Account>(Account.class),
                    accountName);
